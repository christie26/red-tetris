...
describe("ResultBox Component", () => {
  test("renders waiting message when status is 'waiting'", () => {
    ...
    expect(
      screen.getByText(/they are playing\. you should wait until it ends\./i),
    ).toBeInTheDocument();
  });
  ...
  test("renders leader's ready message when status is 'ready' and user is leader", () => {
    ...
    expect(
      screen.getByText(/you can click button to start a game/i),
    ).toBeInTheDocument();
  });
  ...
  test("renders non-leader's ready message when status is 'ready' and user is not leader", () => {
    ...
    expect(
      screen.getByText(/please wait until the leader starts a game/i),
    ).toBeInTheDocument();
  });
  ...
  test("renders win message when user is the winner and status is 'end-play'", () => {
    ...
    expect(
      screen.getByText(/🎉🎊 congrat! you did it! 🏆🎉/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/please wait until the leader starts a game\./i),
    ).toBeInTheDocument();
  ...
  test("renders lose message when user is not the winner and status is 'end-play'", () => {
    ...
    expect(
      screen.getByText(/😅 oops! better luck next time! 🍀/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/please wait until the leader starts a game\./i),
    ).toBeInTheDocument();
  ...
  test("renders error message when status is 'error'", () => {
    ...
    expect(
      screen.getByText(
        /there is already a player with same name\. choose different name and try again\./i,
      ),
    ).toBeInTheDocument();
  });
  ...
  test("renders server waiting message when status is 'waitServer'", () => {
    ...
    expect(
      screen.getByText(
        /waiting for server \.\.\. refresh the page in a few second/i,
      ),
    ).toBeInTheDocument();
  });
